...
class PersonAffectedController extends Controller
{
    ...
    public function show(PersonAffected $personAffected): JsonResponse
    {
        ...
        $centers = EvacuationCenter::query()->with('barangay')->withCount(['activeAssignments', 'personAffecteds'])
        ...
        return response()->json(['data' => [
            ...
            'evacuation_center_assignment' => [
                ...
                'centers' => $centers->map(fn ($center) => [
                    ...
                    'occupied' => $center->active_assignments_count + $center->person_affecteds_count,
                    'is_full' => ($center->active_assignments_count + $center->person_affecteds_count) >= $center->capacity,
                ])->values(),
            ],
    ...
    public function assignEvacuationCenter(Request $request, PersonAffected $personAffected): JsonResponse
    {
        ...
        $center = EvacuationCenter::query()->whereKey($data['evacuation_center_id'])
            ->where('is_active', true)->where('status', 'ACTIVE')->withCount(['activeAssignments', 'personAffecteds'])->first();
