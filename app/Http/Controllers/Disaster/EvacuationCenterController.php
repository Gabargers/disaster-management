...
class EvacuationCenterController extends Controller
{
    ...
    public function payoutDetails(EvacuationCenter $center, AffectedFamily $family): JsonResponse
    {
        ...
        return response()->json(['success'=>true,'data'=>[
            ...
            'payout'=>$payout?['id'=>$payout->id,'status'=>$payout->status,'assistance_kind'=>$payout->assistance_kind,'quantity'=>$payout->quantity,'amount'=>$payout->amount,'provider'=>$payout->provider,'notes'=>$payout->photo_caption,'payout_date'=>$session?->payout_date?->format('Y-m-d'),'released_at'=>$payout->released_at?->toIso8601String(),'released_by'=>$payout->releaser?->name,'has_photo'=>(bool)$payout->payout_photo_path,'photo_url'=>$payout->payout_photo_path?route('disaster.payouts.releases.photo',$payout):null,'can_release'=>$center->payout_availability==='AVAILABLE'&&$payout->status==='Scheduled'&&$family->status===FamilyStatus::PAYOUT_SCHEDULED&&request()->user()->can('manage payout schedules')]:null,
            ...
            'availability'=>['status'=>$center->payout_availability,'can_process'=>$center->payout_availability==='AVAILABLE'&&request()->user()->can('manage payout schedules')],
