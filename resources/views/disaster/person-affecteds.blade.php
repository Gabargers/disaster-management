...
<div class="row g-5 mb-8">
    @foreach([['People received',$totalPeople,'primary','profile-user'],['Status events',$totalEvents,'success','abstract-26'],['Latest API update',$latestReceivedAt ? \Illuminate\Support\Carbon::parse($latestReceivedAt)->format('M d, Y h:i A') : 'No data yet','warning','time']] as [$label,$value,$color,$icon])
...
<div class="card card-flush shadow-sm">
    <div class="card-header align-items-center py-5"><div class="card-title"><div><h3 class="fw-bold mb-1">Person Affected</h3><div class="text-muted fs-7">Resident records received through the TCISS API.</div></div></div><div class="card-toolbar"><span class="badge badge-light-primary">{{$people->total()}} {{Str::plural('record',$people->total())}}</span></div></div>
    <div class="card-body pt-0">
        <form method="GET" class="row g-4 mb-8 align-items-end"><div class="col-md-6"><label class="form-label">Resident or control number</label><input type="search" name="search" value="{{request('search')}}" class="form-control form-control-solid" placeholder="Search resident"></div><div class="col-md-3"><label class="form-label">Latest status</label><select name="status" class="form-select form-select-solid"><option value="">All statuses</option>@foreach($statuses as $option)<option value="{{$option}}" @selected(request('status')===$option)>{{Str::headline($option)}}</option>@endforeach</select></div><div class="col-md-3 d-flex gap-2"><button class="btn btn-primary flex-grow-1">Filter</button>@if(request()->query())<a href="{{route('disaster.person-affecteds.index')}}" class="btn btn-light">Clear</a>@endif</div></form>
        <div class="table-responsive"><table class="table align-middle table-row-dashed gy-5"><thead><tr class="text-gray-600 fw-bold fs-7 text-uppercase"><th>Control Number</th><th>Full Name</th><th>Barangay</th><th>Street</th><th>Status</th><th class="text-end">Action</th></tr></thead><tbody>
        @forelse($people as $person)<tr><td><span class="text-primary fw-semibold">{{$person->control_number}}</span></td><td><span class="fw-bold text-gray-900">{{$person->full_name ?: '—'}}</span></td><td>{{$person->barangay ?: '—'}}</td><td>{{$person->street ?: '—'}}</td><td><span class="badge badge-light-danger">{{Str::headline($person->latestStatus?->status ?? 'Unknown')}}</span></td><td class="text-end"><button type="button" class="btn btn-sm btn-light-primary js-person-details" data-bs-toggle="modal" data-bs-target="#residentModal" data-url="{{route('disaster.person-affecteds.show',$person)}}">Details</button></td></tr>
        @empty<tr><td colspan="6"><div class="text-center py-15"><i class="ki-duotone ki-profile-user fs-3x text-muted"><span class="path1"></span><span class="path2"></span></i><h4 class="mt-4">No person affected records found</h4><p class="text-muted">Records sent by the TCISS API will appear here automatically.</p></div></td></tr>@endforelse
        </tbody></table></div><div class="d-flex justify-content-end mt-5">{{$people->links()}}</div>
    </div>
</div>

<div class="modal fade" id="residentModal" tabindex="-1"><div class="modal-dialog modal-xl modal-dialog-scrollable"><div class="modal-content"><div class="modal-header"><div><div class="text-muted text-uppercase fs-8 fw-bold">Resident Record</div><h2 id="resident-name" class="mb-1">Resident</h2><div id="resident-control" class="text-primary fw-semibold"></div></div><button type="button" class="btn btn-sm btn-icon btn-light" data-bs-dismiss="modal"><i class="ki-duotone ki-cross fs-2"><span class="path1"></span><span class="path2"></span></i></button></div><div class="modal-body"><div id="resident-loading" class="text-center py-15"><span class="spinner-border text-primary"></span></div><div id="resident-content" class="d-none"><div class="row g-5 mb-5"><div class="col-lg-6"><div class="border rounded p-6 h-100"><h3>Personal Information</h3><div id="personal-info" class="detail-grid"></div></div></div><div class="col-lg-6"><div class="border rounded p-6 mb-5"><h3>Address</h3><div id="address-info" class="detail-grid"></div></div><div class="border rounded p-6"><h3>Family Information</h3><div id="family-info" class="detail-grid"></div></div></div></div><div class="border rounded p-6 mb-5"><div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4"><div><h3 class="mb-1">Evacuation Center Assignment</h3><div id="current-center" class="text-muted fs-7">Not assigned</div></div></div><div id="assignment-message"></div><form id="person-center-form" class="row g-3 align-items-end"><div class="col-md-9"><label class="form-label">Evacuation Center</label><select id="person-center-select" name="evacuation_center_id" class="form-select form-select-solid"></select></div><div class="col-md-3"><button id="assign-center-button" type="submit" class="btn btn-primary w-100">Assign Evacuation Center</button></div></form></div><div class="border rounded p-6"><div class="d-flex justify-content-between"><h3>Family Composition</h3><span id="family-count" class="badge badge-light-primary"></span></div><div class="table-responsive"><table class="table table-row-dashed gy-4"><thead><tr><th>Control No.</th><th>Name</th><th>Relationship</th><th>Age</th><th>Sex</th><th>Code</th><th>Housing</th></tr></thead><tbody id="family-rows"></tbody></table></div></div></div></div></div></div></div>
@endsection

@push('styles')
<style>.detail-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem 2rem}.detail-item.wide{grid-column:1/-1}.detail-label{color:#99a1b7;font-size:.78rem;margin-bottom:.15rem}.detail-value{color:#071437;font-weight:500}@media(max-width:575px){.detail-grid{grid-template-columns:1fr}}</style>
@endpush
...
document.getElementById('residentModal').addEventListener('show.bs.modal', async event => {
    ...
    element('resident-name').textContent = 'Resident';
    ...
    try {
        ...
        if (!response.ok) throw new Error('Unable to load resident record.');

        const data = (await response.json()).data;
        element('resident-name').textContent = data.full_name || 'Resident';
        ...
        assignmentMessage.innerHTML = assignment.has_centers
            ? ''
            : `<div class="alert alert-warning">Create an active Evacuation Center first before assigning this resident.${assignment.create_url ? ` <a href="${escapeHtml(assignment.create_url)}" class="alert-link">Go to Evacuation Center</a>` : ''}</div>`;
        ...
        element('family-rows').innerHTML = data.family_members.length
            ? data.family_members.map(member => `<tr><td>${escapeHtml(member.control_number)}</td><td>${escapeHtml(member.full_name)}</td><td>${escapeHtml(member.relationship)}</td><td>${escapeHtml(member.age)}</td><td>${escapeHtml(member.sex)}</td><td>${escapeHtml(member.code)}</td><td>${escapeHtml(member.housing)}</td></tr>`).join('')
            : '<tr><td colspan="7" class="text-center text-muted py-8">No family composition received.</td></tr>';
