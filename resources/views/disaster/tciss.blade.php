...
(() => {
    ...
    document.querySelectorAll('.js-open-tciss').forEach(button => button.addEventListener('click', async () => {
        ...
        try {
            ...
            $('#family-members').html(d.family_members.length ? d.family_members.map((m,i) => `<tr><td>${i+1}</td><td class="fw-bold">${esc(m.name)}</td><td>${date(m.birthdate)}</td><td>${esc(m.age)}</td><td>${esc(m.relationship)}</td><td>${esc(m.sex)}</td><td>${esc(m.occupation)}</td><td>${money(m.monthly_income)}</td><td>${badge(m.health_condition)}</td><td colspan="2">${esc(m.remarks_label)}</td></tr>`).join('') : emptyRow(11));
